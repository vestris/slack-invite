...
describe SlackInvite::App do
  subject do
    SlackInvite::App.instance
  end

  describe '#instance' do
    it 'is an instance of the strava app' do
      expect(subject).to be_a(SlackRubyBotServer::App)
      expect(subject).to be_an_instance_of(SlackInvite::App)
    end
  end
  ...
  context 'subscribed' do
    ...
    let(:plan) { stripe_helper.create_plan(id: 'invite-yearly', amount: 999) }
    let(:customer) do
    ...
    describe '#check_subscribed_teams!' do
      ...
      it 'notifies no active subscriptions' do
        ...
        expect(Stripe::Customer).to receive(:retrieve).and_return(customer)
        expect_any_instance_of(Team).to receive(:inform!).with(text: 'Your subscription was canceled and your team has been downgraded. Thank you for being a customer!')
